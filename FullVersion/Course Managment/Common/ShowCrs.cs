...
namespace Course_Managment.Common
{
    public partial class ShowCrs : Form
    {
        ...
        private void HighRegisteration_Click(object sender, EventArgs e)
        {
            ...
            connection.Open();
            SqlDataAdapter adapter = new SqlDataAdapter("WITH CourseCounts AS (SELECT c.CID, COUNT(e.SID) AS NumStudents FROM ENROLL_IN e JOIN COURSE c ON e.CID = c.CID GROUP BY c.CID),MaxCount AS (SELECT MAX(NumStudents) AS MaxStudents FROM CourseCounts)SELECT c.*FROM COURSE c JOIN CourseCounts cc ON c.CID = cc.CID JOIN MaxCount mc ON cc.NumStudents = mc.MaxStudents;", connection);
